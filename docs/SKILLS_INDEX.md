...
|| 64-bit shim `msm8996.so` DT_NEEDED → 32-bit cesta ❌ | `module/system/vendor/lib64/hw/` + `system/vendor/lib64/hw/` | **FIXED** `2b50c48` (module) + `system/` working copy sync (Aug 14) — `patchelf --replace-needed "/vendor/lib/hw/audio.primary.icx1295.so" "/vendor/lib64/hw/audio.primary.icx1295.so"` | verify: `readelf -d <path> | grep NEEDED` + `md5sum system/vendor/lib64/hw/audio.primary.msm8996.so module/system/vendor/lib64/hw/audio.primary.msm8996.so` → `4804a2b2` (match) |
